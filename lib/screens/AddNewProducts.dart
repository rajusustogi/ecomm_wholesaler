import 'dart:convert';
import 'package:ecomm_wholesaler/Models/SearchProduct.dart';
...
class AddNewProduct extends StatefulWidget {
  ...
  const AddNewProduct({Key key, this.id}) : super(key: key);
  @override
...
class _AddNewProductState extends State<AddNewProduct> {
  ...
  Future<List<SearchData>> _getAllPosts() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    String token = prefs.getString('token');
    var response =
        await http.get(SEARCH + search, headers: {"Authorization": token});
    // print("body" + response.body);
    SearchProduct products = SearchProduct.fromJson(json.decode(response.body));
    return products.data;
  }

  final RoundedLoadingButtonController _btnController =
      new RoundedLoadingButtonController();
  _listofitems(List<SearchData> products, BuildContext context, bool add) {
    return Container(
                              ...
                              RoundedLoadingButton(
                                ...
                                onPressed: () async {
                                  ...
                                  var res =
                                    ...
                                    "wholesaler_id": widget.id.toString(),
                          ...
                          RichText(
                            ...
                            text: TextSpan(
                              children: <TextSpan>[
                                ...
                                TextSpan(
                                  text: products[index].manufacturer + "\n",
                                  style: TextStyle(
                                    color: grey,
                                  ),
                                ),
                                TextSpan(
                                  text: products[index].packSize + "\n",
                                  style: TextStyle(
                                    color: grey,
                                  ),
                                ),
